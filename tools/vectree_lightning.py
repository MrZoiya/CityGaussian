...
class Quantization():
    ...
    def fully_vq_reformat(self):
        ...
        if self.save_path is not None:
            ...
            np.savez_compressed(f'{save_path}/extreme_saving/non_vq_mask.npz',
    ...
    def dequantize(self):
        ...
        if self.no_save_ply == False:
            ...
            checkpoint["state_dict"]["gaussian_model.gaussians.shs_dc"] = dequantized_feats[:, 6:9].reshape(
                dequantized_feats.shape[0], 3, 1).permute(0, 2, 1)
            checkpoint["state_dict"]["gaussian_model.gaussians.shs_rest"] = dequantized_feats[:,
                                                                            9:6 + self.sh_dim].reshape(
                dequantized_feats.shape[0], 3, -1).permute(0, 2, 1)
            checkpoint["state_dict"]["gaussian_model.gaussians.opacities"] = dequantized_feats[:,
                                                                             6 + self.sh_dim:7 + self.sh_dim]
